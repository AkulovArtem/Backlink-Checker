...
- Опциональная проверка индексации доноров в Google через XMLRiver или XMLStock (только страницы с найденным бэклинком)
...
2. Нажмите «Настройки» и вставьте персональный URL XMLRiver и/или XMLStock (в кабинете сервиса). Баланс подтянется сам.
3. На главном экране нажмите «+ Создать задание» и заполните поля. При необходимости включите «Проверка индексации ссылок».
4. Проверка идёт в несколько потоков. Клик по заданию открывает отчёт в реальном времени.
5. Чтобы докинуть ссылки в готовое задание: меню ⋮ → «Добавить ссылки». Проверятся только новые URL, старые результаты останутся.
6. Результаты можно смотреть в программе или выгрузить в Excel.
