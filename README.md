...
- Опциональная проверка индексации в Google через XMLRiver или XMLStock (только доноры с найденным бэклинком, топ-10 выдачи)
- Выбор сервиса прямо в форме задания, с живым балансом; URL задаётся в Настройках
- Полная очистка базы заданий в Настройках (URL сервисов и тема сохраняются)
...
- в Excel — отдельные колонки «В индексе Google» и «Ошибка индекса Google»
...
2. Нажмите «Настройки» и вставьте персональный URL XMLRiver и/или XMLStock (в кабинете сервиса). Баланс подтянется сам. Кнопки: «Отмена» и «Сохранить». Там же можно полностью очистить базу заданий.
3. На главном экране нажмите «+ Создать задание» и заполните поля. Если нужна проверка индексации в Google — включите чекбокс и выберите XMLRiver или XMLStock: рядом виден баланс.
